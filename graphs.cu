...
=======
	for (int current_level = 1; current_level < graph.L_VALUE; current_level++)
	{
			  ...
			  calc_position_in_new_array(temp_from, temp_to, position_in_array,
				...
				std::cout << "Level "<< current_level << " Pre expanded time is " << t1_time/(float)USECPSEC << std::endl;
			  ...
			  if(debug)
				...
				cudaFree(0);
				t1_time = dtime_usec(t1_time);
				std::cout << "Level "<< current_level << " EXPANDER KERNEL " << t1_time/(float)USECPSEC << std::endl;
		  	...
		  	inclusive_scan(device, positions_vertex_current_level,
 	  		...
 	  		if(debug)
				...
				t1_time = dtime_usec(t1_time);
				std::cout << "Level "<< current_level << " Pre unifier calculation " << t1_time/(float)USECPSEC << std::endl;
				// Remove all dublicates. Calculate the opacity, remove if needed
				if (debug)
					std::cout << "Starting unification " << std::endl;
		    ...
		    cudaDeviceSynchronize();
				t1_time = dtime_usec(t1_time);
				std::cout << "Level "<< current_level << " UNIFIER KERNEL " << t1_time/(float)USECPSEC << std::endl;
				// Now we have unique expanded_array
				...
				if(debug)
				  ...
				  /******************************************************* Edge copy kernel start  ********************************************/
					...
					device_free(expanded_array);
					t1_time = dtime_usec(t1_time);
					std::cout << "Level "<< current_level << " Copier KERNEL " << t1_time/(float)USECPSEC << std::endl;
					//device_free(positions_vertex_current_level);
					...
					t1_time = dtime_usec(t1_time);
					std::cout << "Level "<< current_level << " CALC LOPACITY : " << t1_time/(float)USECPSEC << std::endl;
					renew_graph_offset(graph, vertex_ending_offsets, current_level);
					...
					t1_time = dtime_usec(t1_time);
					std::cout << "Level "<< current_level << " Remove Candidates forming : " << t1_time/(float)USECPSEC << std::endl;
					...
					first_level_remove<<<grid_size, BLOCK_SIZE>>>(
					 ...
					 cudaDeviceSynchronize();
					 t1_time = dtime_usec(t1_time);
 					 std::cout << "Level "<< current_level << " FIRST level remove " << t1_time/(float)USECPSEC << std::endl;
					 t1_time = dtime_usec(0);
					 grid_size = (graph.number_of_vertex+BLOCK_SIZE-1)/BLOCK_SIZE;
					 calc_new_offsets<<<grid_size, BLOCK_SIZE>>>(
																		...
																		graph.number_of_vertex);
					 t1_time = dtime_usec(t1_time);
					 std::cout << "Level "<< current_level << " New offset kernel " << t1_time/(float)USECPSEC << std::endl;
...
=======
	...
	order_graph(graph);
	t1_time = dtime_usec(t1_time);
	std::cout << "Time to convert and order + init " << t1_time/(float)USECPSEC << " seconds" << std::endl;
	...
	t1_time = dtime_usec(t1_time);
	std::cout << "Initial lopacity calcualtion " << t1_time/(float)USECPSEC << " seconds" << std::endl;
//	graph.print_opacity_matrix();
//	graph.print_csr_graph();
	...
	if (graph.opacity_index[0] != -1)
	{
		...
		edge_removal(graph);
		t1_time = dtime_usec(t1_time);
		std::cout << "Initial edge removal " << t1_time/(float)USECPSEC << " seconds" << std::endl;
		cudaMemGetInfo(&free_mem,&total_mem);
		...
		order_graph(graph);
		t1_time = dtime_usec(t1_time);
		std::cout << "Secondary convertion " << t1_time/(float)USECPSEC << " seconds" << std::endl;
	}
